...
The rules of `MediaQuery` is: use `_` for `not` and `$` for `only`, that's all, Let's check if it compose:
...
Nice! The `@` rules in one child mss will be float to the global scope while not affecting the parent's other children at all, so you can safely add MediaQuery anywhere you want inside mss, the parse will handle all the rest.

We got a similar `KeyFrames` too:
...
The keys are numbers that will be normalized to pencentage, so you can just write them in proportion. Note there's one thing to be taken care of, `KeyFrames` don't have local namespace, it's a design of css not mss, so you have to make sure the animation name is unique in the mss tree, let's check it out:

mss

```coffee
myAnima = mss.KeyFrames('myKeyFrameAnima') # this name has to be unique!

childrenAnima = myAnima
