...
(function() {
  var Animate, ClearFix$, KeyFrames, LineSize, MediaQuery, Mixin, PosAbs, PosRel, Size, TRAVERSE, TextEllip$, Vendor, bw, gold, goldR, hsl, hsla, isIeLessThan9, mss, num, parse, parsePropName, parseR, parseSelectors, pc, px, reTag, rgb, rgba, tag, unTag, unit;
  ...
  TRAVERSE = function(mss, mssFn, propFn) {
    ...
    for (k in mss) {
      v = mss[k];
      results.push(newMss[k] = typeof v === 'object' ? TRAVERSE(mssFn(k(v)), mssFn, propFn) : propFn(k, v));
    }
    return results;
  };
